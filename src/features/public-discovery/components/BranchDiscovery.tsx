...
export function BranchDiscovery() {
  ...
  useEffect(() => {
    if (!isLoading && !error && branches.length === 1) {
      window.location.replace(`/menu?branch=${encodeURIComponent(branches[0].branchSlug)}`);
    }
  ...
  if (error) {
    return (
      <DiscoveryState
        ...
        onRetry={() => {
          void Promise.all([restaurantQuery.refetch(), branchesQuery.refetch()]);
        }}
      />
    );
  }
    ...
    <main id="main-content" className="mx-auto w-full max-w-6xl px-5 pb-16 sm:px-8 sm:pb-24 lg:px-12">
      <div className="mb-8 max-w-2xl sm:mb-10">
        ...
        <h1 className="font-heading text-[clamp(2.8rem,8vw,5.8rem)] font-semibold leading-[0.9] tracking-[-0.08em] text-[#12324a]">
          ¿Dónde nos <em className="font-normal text-[#e76832]">visitas</em> hoy?
        </h1>
        <p className="mt-5 max-w-xl text-base leading-7 text-[#12324a]/70 sm:text-lg sm:leading-8">
          Selecciona la sucursal para consultar su menú y descubrir la carta disponible.
        </p>
      </div>
