    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-[#dcecef]">
        ...
        <span
          className={`absolute left-4 top-4 rounded-full px-3 py-1.5 text-[0.65rem] font-bold uppercase tracking-[0.13em] shadow-sm ${
            isSoldOut ? 'bg-[#fff0e9] text-[#b34b25]' : 'bg-[#f8fbf8]/95 text-[#22624e]'
          }`}
        >
      ...
      <div className="p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <p className="shrink-0 pt-0.5 text-base font-bold text-[#e76832]">{formatMenuPrice(dish.price)}</p>
        </div>
        <p className="mt-3 text-sm leading-6 text-[#12324a]/68">{dish.description}</p>

        {dish.ingredients.length > 0 ? (
          <div className="mt-5">
            <p className="mb-2 text-[0.65rem] font-bold uppercase tracking-[0.18em] text-[#12324a]/45">Lleva</p>
            <ul className="flex flex-wrap gap-1.5" aria-label={`Ingredientes de ${dish.name}`}>
        ...
        {dish.allergens.length > 0 ? (
          <p className="mt-5 flex items-start gap-2 border-t border-[#12324a]/10 pt-4 text-xs leading-5 text-[#12324a]/60">
            <CircleAlert className="mt-0.5 shrink-0 text-[#e76832]" size={14} aria-hidden="true" />
            <span>
              <strong className="font-semibold text-[#12324a]/80">Alérgenos:</strong>{' '}
              {dish.allergens.join(', ')}
            </span>
          </p>
        ) : null}
